...
from flectra.tools import pycompat, sql
from flectra.addons.base.ir.ir_model import IrModel
...
class BiSQLView(models.Model):
    ...
    action_id = fields.Many2one(
        string='Flectra Action', comodel_name='ir.actions.act_window',
        readonly=True)
    ...
    cron_id = fields.Many2one(
        string='Flectra Cron', comodel_name='ir.cron', readonly=True,
        help="Cron Task that will refresh the materialized view")

    rule_id = fields.Many2one(
        string='Flectra Rule', comodel_name='ir.rule', readonly=True)
